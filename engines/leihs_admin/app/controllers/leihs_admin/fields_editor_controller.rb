module LeihsAdmin
  class FieldsEditorController < AdminController
    ...
    def destroy
      ...
      respond_to { |format| format.json { render(status: :ok, json: {}) } }
    end

    def groups
      props = { groups: Field.unscoped.all.map { |f| f.data['group'] }.uniq }
      respond_to { |format| format.json { render(status: :ok, json: props) } }
    end
    ...
    def new_react
      if Field.unscoped.where(id: params[:field][:id]).first
        respond_to do |format|
          format.json { render(status: :ok, json: { result: 'field-exists-already' }) }
        end
        return
      end
      ...
      field.save!
      respond_to { |format| format.json { render(status: :ok, json: { result: 'field-saved' }) } }
    end
