    ...
    def show
      @list =
        ...
        when 'Who borrowed the most things?'.parameterize
          Statistics::Base.hand_overs([User, Model], params.to_unsafe_hash)
        when 'Which inventory pool is busiest?'.parameterize
          Statistics::Base.hand_overs([InventoryPool, Model], params.to_unsafe_hash)
        when 'Who bought the most items?'.parameterize
          Statistics::Base.item_values([InventoryPool, Model], params.to_unsafe_hash)
        when 'Which item is busiest?'.parameterize
          Statistics::Base.hand_overs([Item, User], params.to_unsafe_hash)
        when 'Which model is busiest?'.parameterize
          Statistics::Base.hand_overs([Model, Item], params.to_unsafe_hash)
        when 'Which inventory pool has the most contracts?'.parameterize
          Statistics::Base.contracts([InventoryPool, User], params.to_unsafe_hash)
        else
