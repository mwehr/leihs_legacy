...
placeholder :whether_providing do
  match('not providing') { false }
  match('providing') { true }
end
    ...
    module BuildingSteps
      ...
      step 'I see a list of buildings' do
        within '.list-of-lines' do
          Building.limit(5).each { |building| find('.row > .col-sm-3', text: building.name) }
        end
      end

      step 'I see a list of all buildings' do
        within '.list-of-lines' do
          Building.all.each { |building| find(".row[data-id='#{building.id}']") }
        end
      end
      ...
      step 'the first row contains rooms count of the building' do
        @first_row ||= first('.list-of-lines .row')
        @first_building ||= Building.find(@first_row['data-id'])
        expect(@first_row).to have_content "#{@first_building.rooms.count} #{_('rooms')}"
      end

      step 'the first row contains items count of the building' do
        @first_row ||= first('.list-of-lines .row')
        @first_building ||= Building.find(@first_row['data-id'])
        expect(@first_row).to have_content "#{@first_building.items.count} #{_('items')}"
      end
      ...
      step 'there is a deletable building' do
        ...
        @building ||=
          FactoryGirl.create(
            :building, name: Faker::Address.street_address, code: Faker::Address.building_number
          )
      ...
      step 'its general room was deleted from the database too' do
        expect(Room.find_by(building_id: @building.id, general: true)).not_to be
      end
    end
  end
end
