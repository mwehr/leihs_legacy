  ...
  module Spec
    module InventoryPoolsSteps
      ...
      step 'the inventory pool is saved' do
        @inventory_pool =
          InventoryPool.find_by_name_and_shortname_and_email('test', 'test', 'test@test.ch')
      ...
      step 'I see the list of all active inventory pools sorted alphabetically' do
        ...
        within '.list-of-lines' do
          expect(InventoryPool.where(is_active: true).sort.map(&:name)).to be ==
            all('.row > .col-sm-6').map(&:text)
        end
      end
      ...
      step 'I change name, shortname and email address' do
        all('.row .col-sm-6 strong', text: _('Name')).first.find(:xpath, './../..').find(
          'input'
        ).set 'test'
        all('.row .col-sm-6 strong', text: _('Short Name')).first.find(:xpath, './../..').find(
          'input'
        ).set 'test'
        all('.row .col-sm-6 strong', text: _('E-Mail')).first.find(:xpath, './../..').find(
          'input'
        ).set 'test@test.ch'
      end
      ...
      step "I don't fill in :input_label" do |input_label|
        expect(
          find('.form-group', text: input_label, match: :prefer_exact).find('input').value
        ).to be_blank
      end
      ...
      step 'multiple inventory pools are granting automatic access' do
        ...
        inventory_pool =
          @current_user.inventory_pools.managed.where.not(automatic_access: true).first
        inventory_pool.update_attributes automatic_access: true if inventory_pool
      ...
      step "the newly created user has 'customer'-level access to all " \
        ...
        expect(
          @user.access_rights.pluck(:inventory_pool_id)
      ...
      step "each line displays the inventory pool's name" do
        within '.list-of-lines' do
          InventoryPool.all.each { |ip| find '.row', match: :prefer_exact, text: ip.name }
        end
      end

      step "each line displays the inventory pool's short name" do
        within '.list-of-lines' do
          InventoryPool.all.each { |ip| find '.row', match: :prefer_exact, text: ip.shortname }
        end
      end
      ...
      step 'the inventory pool has ' \
        ...
        FactoryGirl.create(
          :item, inventory_pool: @inventory_pool, owner: FactoryGirl.create(:inventory_pool)
        )
      end
      ...
      step 'the inventory pool owns ' \
        ...
        FactoryGirl.create(
          :item, inventory_pool: FactoryGirl.create(:inventory_pool), owner: @inventory_pool
        )
      end
      ...
      step 'there exists an inventory pool with :order_type' do |order_type|
        ...
        when 'unsubmitted order'
          FactoryGirl.create(:reservation, status: :unsubmitted, inventory_pool: @inventory_pool)
        when 'submitted order'
          user = FactoryGirl.create(:customer, inventory_pool: @inventory_pool)
          order =
            FactoryGirl.create(
              :order, user: user, inventory_pool: @inventory_pool, state: :submitted
            )
          FactoryGirl.create(
            :reservation, status: :submitted, order: order, inventory_pool: @inventory_pool
          )
        when 'approved order'
          user = FactoryGirl.create(:customer, inventory_pool: @inventory_pool)
          order =
            FactoryGirl.create(
              :order, user: user, inventory_pool: @inventory_pool, state: :approved
            )
          FactoryGirl.create(
            :reservation, status: :approved, order: order, inventory_pool: @inventory_pool
          )
        when 'signed contract'
          FactoryGirl.create(:open_contract, inventory_pool: @inventory_pool)
        end
      end
      ...
      step 'I see an error message regarding the deactivation of inventory pool' do
        expect(find('#flash .error').text).to match /Inventory pool can't be deactivated/
      end
      ...
      step 'the mail templates have been created for the inventory pool' do
        ...
        template_types.each do |name, type|
          mt = MailTemplate.find_by!(name: name, type: type, is_template_template: true)
      ...
      def fill_in_user_information(attrs)
        ...
        if attrs[:lastname]
          find(selector, match: :prefer_exact, text: _('Last name')).find('input').set attrs[
            :firstname
          ]
        end
        if attrs[:firstname]
          find(selector, match: :prefer_exact, text: _('First name')).find('input').set attrs[
            :firstname
          ]
        end
        if attrs[:email]
        ...
        if attrs[:password]
          find(selector, match: :prefer_exact, text: _('Password')).find('input').set attrs[
            :password
          ]
        end
        if attrs[:password_confirmation]
          find(selector, match: :prefer_exact, text: _('Password Confirmation')).find(
            'input'
          ).set attrs[:password_confirmation]
        end
        click_button _('Save')
      end
    end
...
RSpec.configure do |config|
  config.include(LeihsAdmin::Spec::InventoryPoolsSteps, leihs_admin_inventory_pools: true)
end
