...
const CreateItem = createReactClass({
  ...
  _recursiveFieldModels(fieldModel, fieldModels) {
    if (fieldModel.dependents && fieldModel.dependents.length > 0) {
      return _.reduce(
        ...
        (result, dependent) => {
          return result.concat(this._recursiveFieldModels(dependent, result))
        },
  ...
  _flatFieldModels() {
    return _.reduce(
      ...
      (result, fieldModel) => {
        return result.concat(this._recursiveFieldModels(fieldModel, []))
      },
      []
    )
  },
  ...
  _fieldModelsForSubmit() {
    return _.filter(this._flatFieldModels(), fieldModel => this._isFieldModelForSubmit(fieldModel))
  },
  ...
  _newAttachementFiles() {
    return _.filter(this._attachmentsFieldModel().value.fileModels, fm => {
      return fm.type == 'new'
    })
  },
  ...
  _save(bypassSerialNumberValidation, copy) {
    ...
    _.each(this._attachmentsFieldModel().value.fileModels, fm => {
      if (fm.delete) {
  ...
  _renderTitleButtons() {
    ...
    return (
      <div className="col1of2 text-align-right">
        ...
        <a className="button grey" href={this.props.return_url ? this.props.return_url : this.props.inventory_path}>
          {_jed('Cancel')}
        </a>
