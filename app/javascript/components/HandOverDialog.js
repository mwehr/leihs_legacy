...
const HandOverDialog = createReactClass({
  ...
  _renderDropdownLine(user) {
    return (
      <li key={user.id} className="separated-bottom exclude-last-child ui-menu-item">
        <a
          onClick={event => this._onUserClick(event, user)}
          className="row ui-menu-item-wrapper"
          title="Christophe Besch"
          id="ui-id-62"
          tabIndex="-1">
          <div className="row text-ellipsis">
            <strong>{user.name}</strong>
          </div>
          <div className="row text-ellipsis">{this._userAddress(user)}</div>
        </a>
      </li>
    )
  },
  ...
  _dropdownData() {
    ...
    return _.filter(this.state.userData, u => {
  ...
  _renderDropdown() {
    ...
    return (
      <ul
        ref={ref => (this.ulReference = ref)}
        id="ui-id-1"
        tabIndex="0"
        className="ui-menu ui-widget ui-widget-content ui-autocomplete ui-front ui-autocomplete-disabled"
        style={{ display: display, top: '30px', left: '0px', width: '231px' }}>
      ...
      <div className="emboss white padding-inset-xxs">
        <div className="row">
          <p className="paragraph-s">
            <strong>{this.state.delegatedUser.name}</strong>
          </p>
          <div className="position-absolute-topright padding-inset-xxs">
            <a onClick={this._onClearUser} className="grey padding-inset-xxs" id="remove-user">
              <i className="fa fa-times-circle icon-m" />
            </a>
          </div>
        </div>
      </div>
        ...
        <div className="row margin-bottom-l">
          <div className="col1of3" id="contact-person">
            <input
              ...
              onChange={this._onChangePersonInput}
              ...
              type="text"
            />
            <div id="selected-user">{this._renderDelegatedUser()}</div>
  ...
  _renderAddPurpose() {
    if (this._showAddPurposeButton()) {
      return (
        <button onClick={this._onAddPurpose} className="button inset" id="add-purpose">
          {_jed('Add Purpose')}
        </button>
      )
    } else {
    ...
    if (this._purpose()) {
      return (
        <div className="row margin-bottom-s emboss padding-inset-s">
          ...
          <div className="col1of4 text-align-right">{this._renderAddPurpose()}</div>
        </div>
      )
    } else {
      return null
  ...
  _subreservationQuantity(reservation) {
    return _.reduce(reservation.subreservations, (mem, r) => mem + r.quantity, 0)
  },
    ...
    return (
      <div key={'reservation_' + index} className="row">
        <div className="col1of8 text-align-center">
          <div className="paragraph-s">{this._reservationQuantity(reservation)}</div>
        </div>
      ...
      <div>
        <div className="modal-header row">
          <div className="col3of5">
            ...
            <h3 className="headline-m light">{this._username()}</h3>
          </div>
        ...
        <div className="row margin-top-s padding-horizontal-l">
          <div className="separated-bottom padding-bottom-m margin-bottom-m">
            ...
            <div className={this._renderProvidePurposeClass()} id="purpose-input">
              ...
              <textarea className="row height-xs" id="purpose" name="purpose" />
            </div>
          </div>
          <div className="modal-body">{this._renderLines()}</div>
