...
export const SearchResult = createReactClass({
  ...
  _itemColumn1ModelLabel(modelLabel, isParent) {
    ...
    if (isParent) {
      packageText = <div className="grey-text">Package</div>
    }
  ...
  _itemColumn1(item, isParent, childCount, isChild, modelLabel) {
    if (modelLabel) {
      if (isChild) {
        return <div className="col1of5 line-col" />
      } else {
  ...
  _itemColumn23(item, isChild) {
    ...
    return (
      <div className="col2of5 line-col text-align-left">
        ...
        {to_s}
        <div className="row grey-text">{text3}</div>
      </div>
    )
  },
  ...
  _itemsForModel(type, searchResult, modelId, is_package, modelLabel) {
    if (is_package) {
      ...
      return searchResult.inventory.items
        .filter(item => {
          return item.model_id == modelId && !item.parent_id
        })
        .map(item => {
  ...
  _searchResultModel(type, searchResult, data, is_package) {
    ...
    if (itemCount > 0) {
      downArrow = <i className={'arrow ' + arrowDirection} />
    ...
    return (
      <div
        ...
        data-id="8e24ecf3-ca2e-5526-9dc1-582b6d0084fe"
        ...
        <div className="table" key="result">
          <div className="table-row">
            <div className="table-cell list-of-lines even separated-top padding-bottom-s min-height-l" id="inventory">
              ...
              <h3 className="headline-s light padding-inset-xl text-align-center">{_jed('No entries found')}</h3>
              <div className="height-s" />
            </div>
          </div>
        </div>
