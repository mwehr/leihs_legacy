...
export const SearchMask = createReactClass({
  ...
  _renderCross(selectedValue, top) {
    ...
    return (
      <a
        onClick={event => this.props._onDeselect(event, selectedValue.field)}
        className="font-size-m link grey padding-inset-xs"
        data-placement="top"
        data-toggle="tooltip"
        data-type="remove-field"
        title="Dieses Feld beim Editieren von Gegenständen nicht mehr anzeigen">
  ...
  _renderField(selectedValue, top) {
    var dependencyValue = _.first(
      this.props.selectedValues.filter(other => {
    ...
    return (
      <div key={selectedValue.field.id} id={selectedValue.field.id}>
        <div
          className="white field row emboss padding-inset-xs margin-vertical-xxs margin-right-xs"
          data-editable="true"
          data-id="properties_ankunftsdatum"
          data-required=""
          data-type="field">
          <div className="row">
  ...
  _colLeftOrRight(leftOrRight) {
    return this.props.selectedValues
      ...
      .map(selectedValue => {
        return this._renderField(selectedValue, true)
      })
  },
  ...
  _noFieldsChosen() {
    if (this.props.selectedValues.length == 0) {
      return (
        <h3 className="headline-s light padding-inset-m text-align-center" id="no-fields-message">
  ...
  render() {
    return (
      <div className="row margin-top-l padding-inset-m separated-bottom" style={{ borderBottom: '0px' }}>
        ...
        <form
          onSubmit={this.props.preventSubmit}
          className="row emboss deep margin-top-m padding-inset-s"
          id="field-selection">
