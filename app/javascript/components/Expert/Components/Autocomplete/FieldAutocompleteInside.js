...
export const FieldAutocompleteInside = createReactClass({
  propTypes: {},
    ...
    return this.props.result.map((row, index) => {
      ...
      return (
        <li
          ...
          id="ui-id-227"
          tabIndex="-1"
          onMouseDown={event => this._onSelect(event, row)}>
          <a>
  ...
  _ul() {
    ...
    return (
      <ul
        ...
        tabIndex="0"
        style={{ display: 'block', width: this.props.dropdownWidth }}>
        {this._lis()}
      </ul>
    )
  },
  ...
  _dropdown() {
    if (!this.props.result) {
      ...
      return <div style={{ position: 'relative' }}>{this._ul()}</div>
    }
  },
