...
export const DatePicker = createReactClass({
  ...
  _renderNumber(index) {
    ...
    if (this.props.value) {
      ...
      selected =
        dayMonthYear.year == this.state.year && dayMonthYear.month == this.state.month && dayMonthYear.day == index
    }
    ...
    if (index >= 0 && index < this._daysInMonth()) {
      ...
      if (selected) {
        return (
          <td key={index} className="ui-datepicker-days-cell-over ui-datepicker-today">
            <a
              style={todayStyle}
              className="ui-state-default ui-state-highlight"
              href="#"
              onClick={event => this._select(event, index)}>
              {index + 1}
            </a>
          </td>
        )
        ...
        return (
          <td key={index} className="">
            <a style={todayStyle} className="ui-state-default" href="#" onClick={event => this._select(event, index)}>
              {index + 1}
            </a>
          </td>
        )
      ...
      return (
        <td key={index} className="ui-datepicker-other-month ui-datepicker-unselectable ui-state-disabled">
          &nbsp;
        </td>
      )
    }
  ...
  _renderTable() {
    ...
    return this._interval(rowCount).map(row => {
      return <tr key={row}>{this._renderCols(row)}</tr>
    })
  },
    ...
    return (
      <div
        ...
        <div className="ui-datepicker-header ui-widget-header ui-helper-clearfix ui-corner-all">
          <a className="ui-datepicker-prev ui-corner-all" title="<" onClick={this._previous}>
            <span className="ui-icon ui-icon-circle-triangle-w">&lt;</span>
          </a>
          <a className="ui-datepicker-next ui-corner-all" title=">">
            <span className="ui-icon ui-icon-circle-triangle-e" onClick={this._next}>
              &gt;
            </span>
          </a>
          <div className="ui-datepicker-title">
            <span className="ui-datepicker-month">{this._getMonthText()}</span>&nbsp;<span className="ui-datepicker-year">{this._getYear()}</span>
          </div>
        </div>
