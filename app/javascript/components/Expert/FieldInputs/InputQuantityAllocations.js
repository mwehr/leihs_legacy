...
export const InputQuantityAllocations = createReactClass({
    ...
    if (allocation.deleted) {
      return (
        <div key={'key_' + index} className="row line font-size-xs focus-hover-thin striked" data-type="inline-entry">
          ...
          <div className="line-col col2of10">
            <input
              onChange={event => this._onChangeQuantity(event, index)}
              value={allocation.quantity}
              className="width-full small text-align-center"
              data-quantity-allocation="true"
              name="item[properties][quantity_allocations][][quantity]"
              type="text"
            />
          </div>
          ...
          <div className="line-col col5of10">
            <input
              onChange={event => this._onChangeLocation(event, index)}
              value={allocation.location}
              className="width-full small text-align-center"
              data-room-allocation="true"
              name="item[properties][quantity_allocations][][room]"
              type="text"
            />
          </div>
          <div className="line-col col1of10">
            <button
              onClick={event => this._undoRemoveAllocation(event, index)}
              className="button inset small"
              data-remove="">
              rückgängig
            </button>
          </div>
      ...
      return (
        <div key={'key_' + index} className="row line font-size-xs focus-hover-thin" data-type="inline-entry">
          ...
          <div className="line-col col1of10">
            <button
              onClick={event => this._removeAllocation(event, index)}
              className="button inset small"
              data-remove="">
              Remove
            </button>
  ...
  _renderRows() {
    return this.props.selectedValue.value.allocations.map((allocation, index) => {
      return this._renderRow(allocation, index)
    })
  },
  ...
  render() {
    ...
    return (
      <div
        ...
        data-editable="true"
        data-id="properties_quantity_allocations"
        data-required=""
        data-type="field">
        <div className="row">
          ...
          <div className="col1of2" data-type="value">
            <div className="row">
              <div className="col7of8 padding-vertical-xs" id="remaining-total-quantity">
                {remainingText}
              </div>
        ...
        <div className="list-of-lines even">{this._renderRows()}</div>
      </div>
    )
  }
})
