export const FieldsDropdownData = {
  _onlyMainFields(allFields) {
    return allFields.filter(field => {
      return (
        !field['visibility_dependency_field_id'] && !field['values_dependency_field_id'] && field.id != 'attachments'
      )
    })
  },
  ...
  _determineData(allFields, selectedValues, term) {
    return this._determineFields(allFields, selectedValues, term)
      .sort((a, b) => _jed(a.label).localeCompare(_jed(b.label)))
      .map(field => {
        return {
          id: field.id,
          label: field.label
