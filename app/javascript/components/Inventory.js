...
const Inventory = createReactClass({
    ...
    return (
      <div className="margin-top-l padding-horizontal-m">
        <div className="row">
          ...
          <div className="col2of3">{this._renderActions()}</div>
        </div>
      </div>
    )
  },
  ...
  _renderSubTab(label, value) {
    ...
    return (
      <a
        onClick={event => this._onSubTabClick(event, value)}
        className={className}
  ...
  _renderSubTabs() {
    return (
      <div className="inline-tab-navigation" id="list-tabs">
        ...
        {this._renderSubTab(_jed('Models'), {
          type: 'item',
          packages: 'false'
        })}
        {this._renderSubTab(_jed('Packages'), {
          type: 'item',
          packages: 'true'
        })}
    ...
    if (!ids.length > 0) {
      ...
      App.License
        ...
        .done(data => {
          ...
          var children = _.flatten(packages.map(p => p.children().all()))
  ...
  _loadCategories() {
    ...
    App.CategoryLink.ajaxFetch().done(data => {
      this.setState({
        categoryLinks: data.map(d => App.CategoryLink.find(d.id))
      })
    })
  },
        ...
        <div className="row">
          <div className="col1of6 padding-right-xs">
            <button
              ...
              className="button inset width-full height-full no-padding text-align-center"
              id="categories-toggle">
              <i className="fa fa-reorder vertical-align-middle" />
            </button>
          </div>
  ...
  _filteredCategories(term) {
    return _.filter(App.Category.all(), c => c.name.match(RegExp(term, 'i')))
  },
  ...
  _renderCategoryLine(c) {
    return (
      <a
        onClick={e => this._onCategoryLine(e, c)}
        key={'category_' + c.id}
        className="links black row focus-hover-thin font-size-m padding-horizontal-s padding-vertical-xs round-border-on-hover"
        data-id={c.id}
    ...
    if (this.state.categoriesPath.length > 1 && !this.state.searchMode) {
      ...
      return (
        <a
          ...
          className="emboss links black row focus-hover-thin font-size-m padding-horizontal-s padding-vertical-xs round-border-on-hover"
          data-id={c.id}
          data-type="category-root">
  ...
  _renderRootCategory() {
    return <div id="category-root">{this._renderRootCategoryContent()}</div>
  },
    ...
    if (this.state.categoriesPath.length > 0 && !this.state.searchMode) {
      ...
      return (
        <a
          ...
          className="emboss links black row focus-hover-thin font-size-m padding-horizontal-s padding-vertical-xs round-border-on-hover"
          data-id={c.id}
          data-type="category-current">
          <i className="arrow left" /> {c.name}
        </a>
      )
    } else {
      return null
    ...
    return (
      <div className={classes} id="categories">
        <div className="row padding-inset-s">
          <input
            onChange={this._onCategoriesTerm}
            value={this.state.categoriesTerm}
            autoComplete="off"
            className="small"
            id="category-search"
            placeholder={_jed('Search Category')}
            type="text"
          />
        </div>
  ...
  _hideUsedSelect() {
    return (
      this.state.retired != '' ||
      this.state.is_borrowable != '' ||
      this.state.responsible_inventory_pool_id != '' ||
      this.state.owned ||
      this.state.in_stock ||
          ...
          <div className="col1of4 padding-right-xs">
            <InventoryFilterSelect
              ...
              values={[
                {
                  value: '',
                  label: _jed('retired') + ' & ' + _jed('not retired')
                },
          ...
          <div className="col1of4 padding-right-xs">
            <InventoryFilterSelect
              ...
              values={[
                {
                  value: '',
                  label: _jed('borrowable') + ' & ' + _jed('unborrowable')
                },
              ...
              <div className="col1of5 padding-right-xs">
                <label className="button inset white width-full height-xxs" htmlFor="owned" style={checkboxesStyle}>
                  <input
                    ...
                    name="owned"
                    type="checkbox"
                    onChange={event => this._onCheckboxChange(event, 'owned')}
                  />
              ...
              <div className="col1of5 padding-right-xs">
                <label className="button inset white width-full height-xxs" htmlFor="broken" style={checkboxesStyle}>
                  <input
                    ...
                    name="broken"
                    type="checkbox"
                    onChange={event => this._onCheckboxChange(event, 'broken')}
                  />
                  <span>{_jed('Broken')}</span>
                </label>
              </div>
              <DatePickerWithInput
                value={this.state.before_last_check}
                onChange={this._onDateChange}
                customRenderer={this._customRenderer}
              />
            </div>
          </form>
        </div>
      ...
      <div className="col1of5 padding-right-xs">
        <label className="row">
          <input
            ...
            name="before_last_check"
            placeholder="Inventur vor"
            type="text"
            id="dp1509973221981"
          />
          <span className="addon" onClick={props.onFocus}>
  ...
  _renderResultLoading() {
    return this._renderLoadingOrNothing(
      <img
        className="margin-horziontal-auto margin-top-xxl margin-bottom-xxl"
        src="/assets/loading-4eebf3d6e9139e863f2be8c14cad4638df21bf050cea16117739b3431837ee0a.gif"
      />
    )
  },
  ...
  _renderArrow(model) {
    if (this._itemCount(model) == 0) {
      ...
      return <i className="arrow right" />
    }
  },
  ...
  _renderModelPackage(model) {
    ...
    return <div className="grey-text">{_jed('Package')}</div>
  },
    ...
    if (this._hasEditRights(model)) {
      return (
        <div className="multibutton width-full text-align-right">
          <a
            className="button white text-ellipsis col4of5 negative-margin-right-xxs"
            href={this._modelEditLink(model)}
            title={editLabel}>
            {editLabel}
          </a>
        ...
        <div className="col1of5 line-col">
          <div className="row">
            <div className="col1of2">
              <button
                onClick={event => this._onToggleModel(event, model)}
                className="button inset small width-full"
                data-type="inventory-expander"
                title="Packages">
                {this._renderArrow(model)} <span>{this._itemCount(model)}</span>
              </button>
            </div>
  ...
  _itemEditLink(item) {
    return (
      ...
      '/edit' +
      '?' +
      $.param({ returnUrl: App.Inventory.url() })
    )
  },
  ...
  _renderItemDetail(item) {
    if (item.parent_id) {
      ...
      return <div className="row grey-text">{this._itemLocation(item)}</div>
    }
  },
