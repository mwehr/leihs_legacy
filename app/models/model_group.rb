class ModelGroup < ApplicationRecord
  ...
  has_many :parent_links, class_name: ::ModelGroupLink, foreign_key: :child_id

  has_many :child_links, class_name: ::ModelGroupLink, foreign_key: :parent_id

  has_and_belongs_to_many :children,
                          join_table: :model_group_links,
                          class_name: 'ModelGroup',
  ...
  def all_models
    Model.joins(:model_links).where(model_links: { model_group_id: self_and_descendants.map(&:id) })
      .distinct
  end
  ...
  scope :roots,
        (lambda do
          joins(
            'LEFT JOIN model_group_links AS mgl ' \
  ...
  def label(parent_id = nil)
    if parent_id
      ModelGroupLink.where(child_id: self.id, parent_id: parent_id).first.try(:label) || name
    else
      name
    end
  end
