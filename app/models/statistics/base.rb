    ...
    class << self
      def hand_overs(klasses, options = {})
        ...
        query.map do |x|
          ...
          unless klasses.empty?
            h[:children] = hand_overs(klasses, options.merge(klass.name.foreign_key.to_sym => x.id))
          end
          h
        end
      end
      ...
      def contracts(klasses, options = {})
        ...
        query.map do |x|
          ...
          unless klasses.empty?
            h[:children] = contracts(klasses, options.merge(klass.name.foreign_key.to_sym => x.id))
          end
          h
        end
      end
      ...
      def item_values(klasses, options = {})
        ...
        query.map do |x|
          ...
          unless klasses.empty?
            h[:children] =
              item_values(klasses, options.merge(klass.name.foreign_key.to_sym => x.id))
          end
          h
        end
      end
      ...
      def prepare_query_for_item_values(klass, items, options)
        query =
          klass.unscoped.select(
            "#{klass.name.tableize}.id, " \
        ...
        query =
          case klass.name
          when 'InventoryPool'
            query.joins(:own_items).group('items.owner_id').group('inventory_pools.id').select(
              'inventory_pools.name AS label'
            )
          when 'Model'
            query.joins(:items).group("items.#{klass.name.foreign_key}").group('models.id').select(
        ...
        unless options[:start_date].blank?
          query = query.where items[:created_at].gteq Date.parse(options[:start_date]).to_s(:db)
        end
        unless options[:end_date].blank?
          query = query.where items[:created_at].lteq Date.parse(options[:end_date]).to_s(:db)
        end
        query
      end
      ...
      def get_query_for_contracts(klass, reservations, options)
        ...
        query =
          case klass.name
          when 'User'
            query.joins(reservations: :contract).group("reservations.#{klass.name.foreign_key}")
              .group('users.id')
              .select(
              <<~SQL
                                    CONCAT_WS(' ',
          ...
          when 'InventoryPool'
            query.joins(reservations: :contract).group('inventory_pools.id').select(
              'inventory_pools.name AS label'
            )
          else
            raise "#{klass} not supported"
        ...
        unless options[:inventory_pool_id].blank?
          query = query.where(reservations: { inventory_pool_id: options[:inventory_pool_id] })
        end
        unless options[:model_id].blank?
        ...
        unless options[:start_date].blank?
          query =
            query.where reservations[:start_date].gteq Date.parse(options[:start_date]).to_s(:db)
        end
        unless options[:end_date].blank?
          query.where reservations[:returned_date].lteq Date.parse(options[:end_date]).to_s(:db)
        end
      end
      ...
      def get_initial_query_for_hand_overs(klass, reservations, options)
        klass.unscoped.select(
          ...
          .where(
          reservations[:type].eq('ItemLine').and(reservations[:item_id].not_eq(nil)).and(
            reservations[:returned_date].not_eq(nil)
          )
        )
          .order('quantity DESC')
          .limit(options[:limit])
      ...
      def extend_query_for_hand_overs(query, klass)
        case klass.name
        when 'User'
          query.joins(:reservations).group('users.id').group(
            "reservations.#{klass.name.foreign_key}"
          )
            .select(
            "CONCAT_WS(' ', " \
              'users.firstname, ' \
        ...
        when 'InventoryPool'
          query.joins(:reservations).group('inventory_pools.id').select(
            'inventory_pools.name AS label'
          )
        when 'Model'
          query.joins(:reservations).group('models.id').group(
            "reservations.#{klass.name.foreign_key}"
          )
            .select(
            "CONCAT_WS(' ', " \
        ...
        when 'Item'
          query.joins(item_lines: :model).group('items.id').group(
            "reservations.#{klass.name.foreign_key}"
          )
