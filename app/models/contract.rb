class Contract < ApplicationRecord
  ...
  before_create do
    ...
    self.compact_id ||=
      (3..26).lazy.map { |i| b32[0..i] }.map { |c_id| !Contract.find_by(compact_id: c_id) && c_id }
        .find(&:itself)
  end
  ...
  has_many :reservations, ORDER_BY, dependent: :destroy
  has_many :item_lines, ORDER_BY, dependent: :destroy
  has_many :option_lines, ORDER_BY, dependent: :destroy
  ...
  validate do
    if reservations.empty?
      errors.add(
        :base,
        _(
          'This contract is not signable because ' \
      ...
      if reservations.any? { |line| not [:signed, :closed].include?(line.status) }
        errors.add(
          :base,
          _(
            'The assigned contract reservations have to be ' \
      ...
      if reservations.where(status: :signed).any? { |l| l.end_date < Time.zone.today }
        errors.add(:base, _('End date of a take back line can not be in the past.'))
      end
  ...
  scope :no_verification_required,
        (lambda do
          where.not(
            id: Contract.unscoped.joins(:reservations).with_verifiable_user_and_model.select(:id) # have to be used here, `Contract` uses current scope (WTF) !!!
          )
        end)

  def to_be_verified?
    Contract.joins(:reservations).with_verifiable_user_and_model.where(id: id).exists?
  end
  ...
  scope :filter2,
        (lambda do |params, user = nil, inventory_pool = nil, paginate: true|
          ...
          contracts =
            contracts.joins(:reservations).scope_if_presence(
              params[:status]
            ) { |contracts, states| contracts.where(state: states) }
              .scope_if_presence(params[:search_term]) do |contracts, search_term|
              contracts.search(params[:search_term])
            end
              .scope_if_presence(params[:id]) { |contracts, ids| contracts.where(id: ids) }
              ...
              .scope_if_presence(params[:global_contracts_search], &:sort_for_global_search)
              .distinct

          paginate ? contracts.default_paginate(params) : contracts
        end)
  ...
  def self.search(query)
    ...
    query.split.map(&:strip).each do |q|
      ...
      sql =
        sql.where(
          self.arel_table[:compact_id].matches(qq).or(User.arel_table[:login].matches(qq)).or(
            User.arel_table[:firstname].matches(qq)
          )
  ...
  def self.sign!(current_user, current_inventory_pool, user, selected_lines, purpose, note = nil, delegated_user_id = nil)
    transaction do
      contract =
        Contract.new(
          state: :open, purpose: purpose, user: user, inventory_pool: current_inventory_pool
        )
      contract.note = note

      selected_lines.each do |cl|
        attrs = {
          contract: contract, status: :signed, user: user, handed_over_by_user_id: current_user.id
        }
