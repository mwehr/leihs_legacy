class MailTemplate < ApplicationRecord
  ...
  after_save { destroy if body.blank? }
  ...
  def self.available_liquid_variables_for_order
    [
      ...
      'email_signature', {
        'reservations' => ['l.quantity', 'l.model_name', 'l.start_date', 'l.end_date']
      },
      'comment',
      'purpose'
    ]
  ...
  def self.liquid_variables_for_order(order, comment = nil)
    {
      ...
      inventory_pool: {
        name: order.inventory_pool.name, description: order.inventory_pool.description
      },
      email_signature: Setting.first.email_signature,
      reservations:
        order.reservations.map do |l|
          {
            quantity: l.quantity,
  ...
  def self.liquid_variables_for_user(user, inventory_pool, reservations)
    {
      user: { name: user.name },
      inventory_pool: { name: inventory_pool.name, description: inventory_pool.description },
      email_signature: Setting.first.email_signature,
      reservations:
        reservations.map do |l|
          {
            quantity: l.quantity,
            model_name: l.model.name,
  ...
  def self.get_template(inventory_pool, name, language)
    MailTemplate.find_by!(
      inventory_pool_id: inventory_pool.id, name: name, language: language, format: 'text'
    )
  end
