  ...
  module SerialNumberValidation
    ...
    included do
      ...
      def unique_serial_number?
        not Item.where.not(id: id).where(
          "lower(replace(serial_number, ' ', '')) = lower(replace(?, ' ', ''))", serial_number
        )
          .exists?
      end
      ...
      after_initialize { @skip_serial_number_validation = false }

      private

      def validates_serial_number
        if serial_number and not skip_serial_number_validation and not unique_serial_number?
