class Mailer::Order < ActionMailer::Base
  ...
  def approved(order, comment, sent_at = Time.zone.now)
    ...
    mail(
      ...
      format.text do
        name = 'approved'
        template = MailTemplate.get_template(order.inventory_pool, name, order.target_user.language)
        Liquid::Template.parse(template.body).render(
          MailTemplate.liquid_variables_for_order(order, comment)
        )
      end
    end
  end
  ...
  def submitted(order, sent_at = Time.zone.now)
    ...
    mail(
      ...
      format.text do
        ...
        template = MailTemplate.get_template(order.inventory_pool, name, order.target_user.language)
        Liquid::Template.parse(template.body).render(
          MailTemplate.liquid_variables_for_order(order, nil)
        )
      end
    end
  end
  ...
  def received(order, sent_at = Time.zone.now)
    ...
    choose_language_for(order)
    mail(
      to: (order.inventory_pool.email || settings.default_email),
