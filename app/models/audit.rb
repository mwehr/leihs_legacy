class Audit < Audited::Audit
  ...
  def self.filter(start_date: nil, end_date: nil, auditable_id: nil, auditable_type: nil, search_term: nil, user_id: nil)
    Audit.scope_if_presence(auditable_type) do |audits, auditable_type|
      ...
      .scope_if_presence(end_date) { |audits, end_date| audits.filter_before_end_date(end_date) }
      .scope_if_presence(search_term) { |audits, search_term| audits.filter_by_term(search_term) }
      .scope_if_presence(user_id) { |audits, user_id| audits.where(user_id: user_id) }
  end
