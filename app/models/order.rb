class Order < ApplicationRecord
  ...
  class << self
    ...
    def with_some_line_not_in_any_contract
      where(reservations: { contract: nil }).distinct
    end
  end

  validate do
    unless user.access_right_for(inventory_pool)
      errors.add :base, _('User does not have access to inventory pool: %s') % inventory_pool.name
    end

    if user.access_right_for(inventory_pool).try(&:suspended?) and state != 'rejected'
      errors.add :base, _('User is suspended for inventory pool: %s') % inventory_pool.name
    end

    if reservations.map(&:delegated_user).uniq.count > 1
      errors.add :base, _('An order can not have multiple delegated users.')
    end
  end

  #################################################################################

  scope :with_verifiable_user,
        (lambda do
          where <<-SQL
                      EXISTS (
        SELECT 1
  ...
  scope :no_verification_required,
        (lambda do
          where.not(
            id: Order.unscoped.joins(:reservations).with_verifiable_user_and_model.select(:id) # have to be used here, `Order` uses current scope (WTF) !!!
          )
        end)

  def to_be_verified?
    Order.joins(:reservations).with_verifiable_user_and_model.where(id: id).exists?
  end

  #################################################################################

  scope :filter2,
        (lambda do |params, user = nil, inventory_pool = nil|
          orders =
            if user
              user.orders
          ...
          orders =
            orders.joins(:reservations).with_some_line_not_in_any_contract.scope_if_presence(
              params[:status]
            ) { |orders, states| orders.where(state: states) }
              .scope_if_presence(params[:search_term]) do |orders, search_term|
              orders.search(params[:search_term])
            end
              .scope_if_presence(params[:id]) { |orders, ids| orders.where(id: ids) }
              .scope_if_presence(params[:reservation_ids]) do |orders, reservation_ids|
              orders.where(reservations: { id: reservation_ids }).distinct
            end
              .scope_if_presence(params[:no_verification_required], &:no_verification_required)
              .scope_if_presence(params[:to_be_verified], &:with_verifiable_user_and_model)
              .scope_if_presence(params[:from_verifiable_users], &:with_verifiable_user)
              .scope_if_presence(params[:range].try(:[], :start_date)) do |orders, start_date|
              orders.where('orders.created_at >= ?', start_date)
            end
              .scope_if_presence(params[:range].try(:[], :end_date)) do |orders, end_date|
  ...
  def self.search(query)
    ...
    sql =
      distinct.joins('INNER JOIN users ON users.id = reservations.user_id').joins(
        <<-SQL
    ...
    query.split.map(&:strip).each do |q|
      ...
      sql =
        sql.where(
          User.arel_table[:login].matches(qq).or(User.arel_table[:firstname].matches(qq)).or(
            User.arel_table[:lastname].matches(qq)
          )
  ...
  def send_submitted_notification
    with_logging_send_mail_failure { Notification.order_submitted(self, true) }
  end

  def send_received_notification
    with_logging_send_mail_failure { Notification.order_received(self, true) }
  end
  ...
  def self_approvable?
    errors.add(:base, _('This order has already been approved.')) if state == :approved
    errors.add(:base, _('This user is suspended.')) if user.suspended?(inventory_pool)
    errors.add(:base, _('Please provide a purpose...')) if purpose.blank?
    errors.empty?
  end
  ...
  def min_date
    unless reservations.blank?
      ...
      reservations.min(&:start_date)[:start_date]
  ...
  def max_date
    unless reservations.blank?
      ...
      # rubocop:disable Style/SymbolProc
      reservations.max(&:end_date)[:end_date]
      # rubocop:enable Style/SymbolProc
    end
  end
