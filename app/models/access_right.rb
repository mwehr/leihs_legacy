class AccessRight < ApplicationRecord
  ...
  validate do
    ...
    check_for_existing_reservations if deleted_at
  end
  ...
  before_validation(on: :create) do
    if user
      unless user.access_rights.active.empty?
        if inventory_pool
          old_ar = user.access_rights.active.find_by(inventory_pool_id: inventory_pool.id)
        end
  ...
  scope :suspended,
        (lambda { where.not(suspended_until: nil).where('suspended_until >= ?', Time.zone.today) })
