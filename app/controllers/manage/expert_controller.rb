class Manage::ExpertController < Manage::ApplicationController
  ...
  def index
    respond_to do |format|
      format.json { @inventory = items_for_view(current_inventory_pool, params) }
    end
  end

  def csv_export
    send_data csv_export_expert(current_inventory_pool, params),
  ...
  def excel_export
    send_data excel_export_expert(current_inventory_pool, params),
