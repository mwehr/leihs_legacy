class Manage::VisitsController < Manage::ApplicationController
    ...
    respond_to do |format|
      ...
      format.json do
        ...
        if params[:paginate] == 'false'
          visits =
            current_inventory_pool.visits.filter2(params).includes(user: :notifications).where(
              type: type_param
            )
          ...
          visits_json = visits.as_json(include: [{ user: { methods: :image_url } }, :reservations])
          ...
          visits_json.each do |v|
            ...
            v['reservations'].each do |r|
              object = Model.find_by_id(r['model_id']) || Option.find_by_id(r['option_id'])
              r['model_name'] = object.name
            end

            v['notifications'] = user.notifications.where('created_at >= ?', v['date']).limit(10)
          end
  ...
  def destroy
    ...
    ActiveRecord::Base.transaction { visit.reservations.each(&:destroy!) }
  ...
  def remind
    ...
    grouped_reservations =
      visit.reservations.group_by do |vl|
