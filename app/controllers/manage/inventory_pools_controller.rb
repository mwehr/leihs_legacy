class Manage::InventoryPoolsController < Manage::ApplicationController
  ...
  def daily(date = params[:date])
    if group_manager? and not lending_manager?
      redirect_to manage_orders_path(
                    current_inventory_pool, status: [:approved, :submitted, :rejected]
                  ),
                  flash: params[:flash] and
        return
    end
    ...
    if @date == Time.zone.today
      ...
      @submitted_reservations_count = current_inventory_pool.orders.submitted.to_a.size
    ...
    @hand_overs_count = current_inventory_pool.visits.hand_over.where(date: @date).to_a.size
    @take_backs_count = current_inventory_pool.visits.take_back.where(date: @date).to_a.size
  ...
  def edit
    ...
    @holidays =
      @inventory_pool.holidays.reject { |h| h.end_date < Time.zone.today }.sort_by(&:start_date)
  end
  ...
  def workload(date = params[:date].try { |x| Date.parse(x) })
    ...
    grouped_visits =
      current_inventory_pool.visits.includes(:user).where('date <= ?', today_and_next_4_days.last)
        .group_by { |x| [x.type.to_sym, x.date] }

    chart_data =
      today_and_next_4_days.map do |day|
    ...
    respond_to { |format| format.json { render json: { data: chart_data } } }
  end
  ...
  def setup_holidays_for_render(holidays_attributes)
    if holidays_attributes
      ...
      @holidays =
        @holidays_initial + params_holidays.reject { |h| h[:id] }.map { |h| Holiday.new h }
