class Manage::AvailabilityController < Manage::ApplicationController
  ...
  def index
    ...
    @models.each do |model|
      @availabilities.push id: "#{model.id}-#{user.id}-#{current_inventory_pool.id}",
      changes: model.availability_in(current_inventory_pool).available_total_quantities,
      total_rentable:
        model.items.where(inventory_pool_id: current_inventory_pool).unretired.borrowable.count,
  ...
  def in_stock
    @models.each do |model|
      entitled_in_groups =
        Entitlement.joins(:entitlement_group).where(
      ...
      @availabilities.push id: "#{model.id}-#{current_inventory_pool.id}",
      total_rentable:
        model.items.where(inventory_pool_id: current_inventory_pool).borrowable.unretired.count,
      in_stock:
        model.items.where(inventory_pool_id: current_inventory_pool).borrowable.unretired.in_stock
          .count,
