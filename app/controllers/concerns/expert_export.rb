module ExpertExport
  ...
  included do
    ...
    def order_items_by_models(items)
      items.distinct.select('models.*, items.*').joins(:model).reorder('models.product').order(
        'models.version'
      )
    end

    def header_for_export(objects_for_export)
      objects_for_export.empty? ? [_('No entries found')] : objects_for_export.flat_map(&:keys).uniq
    end

    def csv_export_expert(inventory_pool, params)
      objects = items_for_export(inventory_pool, params).map(&:to_csv_array)
