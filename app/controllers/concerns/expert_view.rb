...
module ExpertView
  ...
  included do
    ...
    def matching_items(inventory_pool, params)
      ...
      items_directly_matching = items.select(:id).where(parent_id: nil)

      parent_items_containing_matching_child = items.select(:parent_id).where.not(parent_id: nil)

      Item.distinct.where(
        <<-SQL
                  items.id IN (#{items_directly_matching
          .to_sql})
          or items.id IN (#{parent_items_containing_matching_child
          .to_sql})
        SQL
      )
    end
    ...
    def items_for_view(inventory_pool, params)
      ...
      result_models =
        InventoryPool.connection.exec_query(query + " limit #{page_size + 1} offset #{start_index}")
          .to_hash
