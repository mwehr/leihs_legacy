class Borrow::ModelsController < Borrow::ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.html do
        @child_categories =
          @category.children.select { |c| @models.from_category_and_all_its_descendants(c).exists? }
        @grand_children = {}
        ...
        @inventory_pools = current_user.inventory_pools.with_borrowable_items.order(:name)
  ...
  def show
    ...
    @inventory_pools =
      current_user.inventory_pools.order(:name).map do |ip|
        {
          inventory_pool: ip,
          workday: ip.workday,
          holidays: ip.holidays.where('CURRENT_DATE <= end_date').order(:end_date),
          total_borrowable:
            @model.total_borrowable_items_for_user_and_pool(
              current_user, ip, ensure_non_negative_general: true
            )
        }
      end
