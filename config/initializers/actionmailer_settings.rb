class ActionMailer::Base
  def self.smtp_settings
    ...
    if ApplicationRecord.connection.tables.include?('settings')
      ...
      begin
        ...
        logger.info(
          "Could not configure ActionMailer because the database doesn't seem to be in the right shape for it. Check the settings table."
        )
        result = default
      end
      ...
      begin
        if (app_settings.smtp_username and app_settings.smtp_password) and
          (!app_settings.smtp_username.empty? and !app_settings.smtp_password.empty?)
          auth_settings = {
  ...
  def self.delivery_method
    begin
      ...
      delivery_method = :test if delivery_method.empty?
    rescue StandardError
