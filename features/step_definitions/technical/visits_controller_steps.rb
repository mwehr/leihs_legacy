...
Given /^test data setup for scenario "Writing an unavailable inventory code"$/ do
  model =
    @inventory_pool.models.detect do |m|
      m.reservations.joins(:contract).where(
        contracts: { inventory_pool_id: @inventory_pool }, returned_date: nil, item_id: nil
      )
        .exists? and
        m.reservations.joins(:contract).where(
          contracts: { inventory_pool_id: @inventory_pool }, returned_date: nil
        )
          .where
          .not(item_id: nil)
          .exists?
  ...
  @line =
    model.reservations.joins(:contract).where(
      contracts: { inventory_pool_id: @inventory_pool }, returned_date: nil, item_id: nil
    )
      .first
...
When /^an unavailable inventory code is assigned to a contract line$/ do
  @response =
    post "/manage/#{@inventory_pool.id}/reservations/#{@line.id}/assign", {
...
When /
       ^the index action of the visits controller is called with the filter parameter "(hand over|take back)" and a given date$
     / do |arg1|
  @date = Date.today
  path =
    case arg1
...
Then /
       ^the result of this action are all (hand over|take back) visits for the given inventory pool and the given date$
     / do |arg1|
