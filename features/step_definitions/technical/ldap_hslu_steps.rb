...
Given(/^the LDAP\-HSLU authentication system is enabled and configured$/) do
  as = AuthenticationSystem.new(name: 'HsluAuthentication', class_name: 'HsluAuthentication')
  as.is_active = true
...
Given(/^an LDAP response object for HSLU is mocked$/) do
  ...
  normal_user_hash = {
    ...
    st: ['Luzern'],
    c: ['CH'],
    objectcategory: ['CN=Person,CN=Schema,CN=Configuration,DC=campus,DC=intern'],
    lastlogoff: ['0'],
  ...
  normal_user_result = [normal_user_entry]

  video_user_hash = {
    samaccountname: ['video_user'],
    mail: ['testbenutzer@doesnot.exist'],
    department: ['Test & Integration'],
    telephonenumber: ['041 999 99 99'],
    samaccounttype: ['805306368'],
  ...
  video_user_result = [video_user_entry]

  numeric_unique_id_user_hash = {
    samaccountname: ['numeric_unique_id_user'],
    mail: ['testbenutzer@doesnot.exist'],
    department: ['Test & Integration'],
    telephonenumber: ['041 999 99 99'],
    displayname: ['DK.BA_VID'],
  ...
  numeric_unique_id_user_result = [numeric_unique_id_user_entry]
  ...
  admin_user_hash = {
    ...
    memberof: [
      'CN=u_all_ad_deny,OU=p_administration,OU=p_group,OU=prod,OU=hslu,DC=campus,DC=intern',
      'CN=blah,OU=diblah,OU=diblahblah,OU=p_ma,OU=p_group,OU=prod,OU=hslu,DC=campus,DC=intern'
    ],
    l: ['Musterstadt'],
    objectclass: ['top', 'person', 'organizationalPerson', 'user'],
    st: ['Luzern'],
  ...
  admin_user_result = [admin_user_entry]

  mocked_ldap = double('mocked_ldap')
  allow(mocked_ldap).to receive(:bind).with(any_args).and_return(true)
  allow(mocked_ldap).to receive(:search).with(
    { base: anything, filter: Net::LDAP::Filter.eq('samaccountname', 'normal_user') }
  )
    .and_return(normal_user_result)
  allow(mocked_ldap).to receive(:search).with(
    { base: anything, filter: Net::LDAP::Filter.eq('samaccountname', 'video_user') }
  )
    .and_return(video_user_result)
  allow(mocked_ldap).to receive(:search).with(
    { base: anything, filter: Net::LDAP::Filter.eq('samaccountname', 'numeric_unique_id_user') }
  )
    .and_return(numeric_unique_id_user_result)
  allow(mocked_ldap).to receive(:search).with(
    { base: anything, filter: Net::LDAP::Filter.eq('samaccountname', 'admin_user') }
  )
    .and_return(admin_user_result)

  allow(Net::LDAP).to receive(:new) { mocked_ldap }
end
