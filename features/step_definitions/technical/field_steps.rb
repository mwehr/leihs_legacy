...
Given /^a user with role (\w+) exists$/ do |manager_role|
  ...
  @user.access_rights <<
    FactoryGirl.create(
      :access_right, role: @manager_role, user: @user, inventory_pool: @inventory_pool
    )
end
...
Then /
       ^the amount of the accessible fields (.*) (\w+) can be different$
     / do |compare_op, higher_manager_role|
  ...
  unless @accessible_fields.empty?
    @accessible_fields.each do |field|
      if field['permissions'] and field['permissions']['role']
        expect(AccessRight::ROLES_HIERARCHY.index(field.data['permissions']['role'])).to be <=
          user_role_level
      end
    end
  end
  ...
  @higher_user.access_rights <<
    FactoryGirl.create(
      :access_right,
      role: higher_manager_role.to_sym, user: @higher_user, inventory_pool: @inventory_pool
    )
  # create also data for an higher level
  @higher_accessible_fields =
    Field.all.select { |f| f.accessible_by? @higher_user, @inventory_pool }
...
Then /^each field provides the value of the item's attribute$/ do
  Field.all.each do |field|
    expected_value =
      Array(field.data['attribute']).inject(@item) do |r, m|
...
Then %r{
       ^each field is capable of providing values even if its values attribute is a lambda\/proc$
     } do
  Field.all.each { |field| expect(field.values).not_to be_nil if field.values }
end
