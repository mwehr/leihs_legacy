...
When /^I follow the sloppy link "([^"]*)" in the greybox$/ do |text|
  ...
  within_frame 'GB_frame' do
    step "I follow the sloppy link undefined#{text}undefined"
  end
end
...
Then /
       ^"([^"]*)" should appear before "([^"]*)"(?: within "([^"]*)")?$
     / do |first, second, selector|
  step 'I wait for the spinner to disappear'
  with_scope(selector) { page.body.index(first).should < page.body.index(second) }
end
