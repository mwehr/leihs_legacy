When /^I pick a closed day for beeing the (start|end) date$/ do |date_target|
  ...
  date = (date + 1.month).beginning_of_month if all("td[data-date='#{date}']").empty?
...
Then /
       ^the (start|end) date becomes red and I see a (closed|not possible|too early) day warning?$
     / do |arg1, arg2|
  within '.modal' do
