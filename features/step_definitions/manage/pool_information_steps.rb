...
When(/^I make a note of which page I'm on$/) { @saved_path = current_path }
  ...
  @table_raw.flatten.each do |field_name|
    within('.row.padding-inset-s', match: :prefer_exact, text: field_name) do
      if field_name == 'Print Contracts'
        ...
        expect(find('input,textarea', match: :first).value).to eq (if field_name == 'E-Mail'
             'test@test.ch'
           else
             'test'
           end)
...
Then(/^I am still on the same page$/) { expect(current_path).to eq @saved_path }
...
When(
  /
    ^I randomly set the workdays monday, tuesday, wednesday, thursday, friday, saturday and sunday to open or closed$
  /
) do
  @workdays = {}
  [0, 1, 2, 3, 4, 5, 6].each do |day|
    select =
      find('.row.emboss', match: :prefer_exact, text: I18n.t('date.day_names')[day]).find(
        'select', match: :first
      )
...
Then(/^the holidays are saved$/) do
  @holidays.each do |holiday|
    expect(
      @current_inventory_pool.holidays.where(
        start_date: holiday[:start_date], end_date: holiday[:end_date], name: holiday[:name]
      )
        .empty?
    ).to be false
  end
...
Then(/^I can delete the holidays$/) do
  ...
  expect(
    @current_inventory_pool.reload.holidays.where(
      start_date: holiday[:start_date], end_date: holiday[:end_date], name: holiday[:name]
    )
      .empty?
  ).to be true
...
When(/^I fill in the following fields in the inventory pool settings:$/) do |table|
  table.raw.flatten.each do |field_name|
    expect(
      find('.row.emboss', match: :prefer_exact, text: field_name).find('input', match: :first).value
        .length
    ).to be > 0
  end
end
...
Then(/^this configuration is saved$/) do
  ...
  step '"Automatic suspension" is enabled'
...
When(/^a user is suspended automatically due to late contracts$/) do
  @user =
    Reservation.where(inventory_pool_id: @current_inventory_pool).signed.where(
      'end_date < ?', Date.today
    )
      .first
      .user
...
Then(
  %r{^they are suspended for this inventory pool until '(\d+)\/(\d+)\/(\d+)'$}
) do |day, month, year|
...
Given(/^there are( no)? users without access right to this inventory pool$/) do |arg1|
  ...
  expect(
    User.all.any? do |user|
      user.access_rights.find_by(inventory_pool_id: @current_inventory_pool).nil?
    end
  ).to be b
end
...
Then(
  /
    ^the newly created user has 'customer'-level access to all inventory pools that grant automatic access(, but not to mine)?$
  /
) do |arg1|
  ...
  ).to match_array @inventory_pools_with_automatic_access.pluck(:id)
  if arg1
    expect(
      @user.access_rights.where('inventory_pool_id != ?', @current_inventory_pool).all? do |ar|
        ar.role == :customer
      end
    ).to be true
  else
...
When(
  /^on the inventory pool I enable the automatic suspension for users with overdue take backs$/
) do
  @current_inventory_pool.update_attributes(
    automatic_suspension: true, automatic_suspension_reason: Faker::Lorem.paragraph
  )
end
...
Then(
  /^the existing suspension motivation and the suspended time for this user are not overwritten$/
) do
...
Then(/^I can change the field "(.*?)"$/) do |arg1|
  ...
  when 'Min. number of days between order and hand over'
    n = rand(0..14)
    find(
      "input[type='number'][name='inventory_pool[workday_attributes][reservation_advance_days]']"
    ).set n
    step 'I save'
    find(
      "input[type='number'][name='inventory_pool[workday_attributes][reservation_advance_days]'][value='#{n}']"
    )
  else
    raise
  end
...
Then(/^I can enter the maximum visits per week day$/) do
  ...
  (0..6).each do |i|
    h[i] = rand(0..14)
    find(
      "input[type='number'][name='inventory_pool[workday_attributes][workdays][#{i}][max_visits]']"
    ).set h[i]
  end
  step 'I save'
  (0..6).each do |i|
    find(
      "input[type='number'][name='inventory_pool[workday_attributes][workdays][#{i}][max_visits]'][value='#{h[
        i
      ]}']"
    )
  end
