...
def enable_manager_for_inventory_pool(user, inventory_pool)
  FactoryGirl.create(
    :access_right, user: user, inventory_pool: inventory_pool, role: 'inventory_manager'
  )
end
...
def shared_prepare_data
  ...
  { user: user, inventory_pool: inventory_pool }
end
...
def check_result(expected_items)
  within('#inventory') do
    ...
    expected_items.each { |item| find("div[data-item-id='#{item.id}']") }
  end
end
...
def fill_type_date(fromTo, field, text_date)
  ...
  input =
    find_field_box(field).find('div.col1of2[data-type=value]').find('div', text: label).find(
      'input'
    )
  input.set('')
...
def fill_type_currency(minMax, field, text_value)
  ...
  find_field_box(field).find('div.col1of2[data-type=value]').find('div', text: label).find('input')
    .set(text_value)
end

Then(/^Step Field Inventory Code$/) { step_field_inventory_code }

Then(/^Step Field Model Id$/) { step_field_model_id }

Then(/^Step Field Serial Number$/) { step_field_serial_number }

Then(/^Step Field Properties Mac Address$/) { step_field_properties_mac_address }

Then(/^Step Field Properties Imei Number$/) { step_field_properties_imei_number }

Then(/^Step Field Name$/) { step_field_name }

Then(/^Step Field Note$/) { step_field_note }

Then(/^Step Fields Retired and Retired Reason$/) { step_field_retired_and_retired_reason }

Then(/^Step Field Is Broken$/) { step_field_is_broken }

Then(/^Step Field Is Incomplete$/) { step_field_is_incomplete }

Then(/^Step Field Is Borrowable$/) { step_field_is_borrowable }

Then(/^Step Field Status Note$/) { step_field_status_note }

Then(/^Step Fields Building Id and Room Id$/) { step_fields_building_id_and_room_id }

Then(/^Step Field Shelf$/) { step_field_shelf }

Then(/^Step Field Is Inventory Relevant$/) { step_field_is_inventory_relevant }

Then(/^Step Field Owner Id$/) { step_field_owner_id }

Then(/^Step Field Last Check$/) { step_field_last_check }

Then(/^Step Field Inventory Pool Id$/) { step_field_inventory_pool_id }

Then(/^Step Field Responsible$/) { step_field_responsible }

Then(/^Step Field Price$/) { step_field_price }
...
def field_config_is_broken
  {
    ...
    :"data" => {
      ...
      :"target_type" => 'item',
      :"values" => [
        { :"label" => 'OK', :"value" => false }, { :"label" => 'Broken', :"value" => true }
      ],
      :"default" => false,
      :"group" => 'Status',
      :"forPackage" => true
...
def field_config_is_incomplete
  {
    ...
    :"data" => {
      ...
      :"target_type" => 'item',
      :"values" => [
        { :"label" => 'OK', :"value" => false }, { :"label" => 'Incomplete', :"value" => true }
      ],
      :"default" => false,
      :"group" => 'Status',
      :"forPackage" => true
...
def field_config_is_borrowable
  {
    ...
    :"data" => {
      ...
      :"type" => 'radio',
      :"values" => [
        { :"label" => 'OK', :"value" => true }, { :"label" => 'Unborrowable', :"value" => false }
      ],
      :"default" => false,
      :"group" => 'Status',
      :"forPackage" => true
    ...
    field_config_price, {
      ...
      :"data" => {
        ...
        :"values" => [
          ...
          }, { :"label" => 'Linux', :"value" => 'linux' }, { :"label" => 'iOS', :"value" => 'ios' }
        ],
        :"permissions" => { :"role" => 'inventory_manager', :"owner" => true },
        :"group" => 'General Information'
      },
