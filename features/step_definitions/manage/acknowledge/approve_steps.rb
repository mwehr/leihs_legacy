...
Given(/^I navigate to the (open orders|hand over visits|take back visits)$/) do |arg1|
  step 'I open the daily view' if current_path != manage_daily_view_path(@current_inventory_pool)
...
Then(/^I see a link to the hand over process of that order$/) do
  find(
    ".line[data-id='#{@contract.id}'] .line-actions a[href='#{manage_hand_over_path(
      @current_inventory_pool, @contract.user
    )}']",
    text: _('Hand Over')
  )
end
...
Then /^I got an information that this contract has problems$/ do
  ...
  expect(error.text).to be == @contract.errors.full_messages.join(' ')
end
