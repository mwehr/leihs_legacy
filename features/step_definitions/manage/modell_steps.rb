  ...
  @model ||=
    begin
      @current_inventory_pool =
        @current_user.inventory_pools.managed.select do |ip|
          not ip.models.empty? and ip.models.any? { |m| m.compatibles.exists? }
        end
          .sample
...
When(/^I remove a compatible model$/) do
  find('.field', match: :first, text: _('Compatibles')).all('[data-remove]').each(&:click)
end
...
Given(/^there is a? (.+) with the following conditions:$/) do |entity, table|
  conditions =
    table.raw.flatten.map do |condition|
      case condition
...
Then(/^all associations have been deleted as well$/) do
  ...
  expect(
    Model.all { |n| n.compatibles.include? Model.find_by_name('Windows Laptop') }.include?(@model)
  ).to be false
end

Then(/^the (?:.+) was deleted from the list$/) do
  [@model, @group, @template].compact.each do |entity|
...
When(/^I remove existing allocations$/) do
  find('.field', match: :first, text: _('Allocations')).all('button', text: _('Remove'))
    .each(&:click)
end

When(/^I add new allocations$/) do
  @groups =
    @current_inventory_pool.entitlement_groups - @model.entitlements.map(&:entitlement_group)

  @groups.each { |group| fill_in_autocomplete_field _('Allocations'), group.name }
end
