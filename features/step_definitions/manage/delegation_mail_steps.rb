...
Given(
  /
    ^there is (an order|a take back|an overdue take back) for a delegation that was not placed by a person responsible for that delegation$
  /
) do |arg1|
  status =
    case arg1
    ...
    @current_user.inventory_pools.managed.detect do |ip|
      User.as_delegations.detect do |delegation|
        if status == :submitted
          @order =
            @contract =
              ip.orders.submitted.where(user_id: delegation).detect do |order|
          ...
          @contract =
            ip.contracts.open.where(user_id: delegation).detect do |c|
              c.delegated_user != delegation.delegator_user
            end
        end
      end
    end
...
Then(/^the approval email is not sent to the delegated user$/) do
  expect(
    (ActionMailer::Base.deliveries.first.to & @contract.user.delegator_user.emails).empty?
  ).to be true
end
