...
Given(/^there is a hand over with at least (\d+) assigned items for a user$/) do |count|
  @hand_over =
    @current_inventory_pool.visits.hand_over.find do |ho|
      ho.reservations.select(&:item).size >= count.to_i
    end
...
Given(/^I open a hand over containing software$/) do
  @hand_over =
    @current_inventory_pool.visits.hand_over.detect do |v|
      v.reservations.any? { |cl| cl.model.is_a? Software }
    end
...
Given(/^there is a hand over with at least one unproblematic model( and an option)?$/) do |arg1|
  ...
  @hand_over =
    @current_inventory_pool.visits.hand_over.detect do |v|
      b =
        v.reservations.select do |line|
          !line.start_date.past? and !line.item and @models_in_stock.include?(line.model)
        end
          .count >=
          1
      b = (b and v.reservations.any? { |line| line.is_a? OptionLine }) if arg1 and b
      b
    end
  expect(@hand_over).not_to be_nil
end
...
Given(
  /^there is a hand over with at least (one problematic line|an item without room or shelf)$/
) do |arg1|
  @hand_over =
    @current_inventory_pool.visits.hand_over.find do |ho|
      ho.reservations.any? do |l|
        if l.is_a? ItemLine
          ...
          when 'an item without room or shelf'
            l.item and
              (l.item.location.nil? or
                (l.item.location.room.blank? and l.item.location.shelf.blank?))
          else
            raise
          end
        end
...
When(/^I assign an inventory code to the unproblematic model$/) do
  @reservation =
    @hand_over.reservations.find do |l|
...
Then(/^the line is selected$/) { find(@line_css).find('input[type=checkbox]:checked') }
...
When(/^I assign an already added item$/) do
  ...
  @number_of_lines_with_item_model = all('.line', text: @reservation.model.name).size
  @line_css = ".line[data-id='#{@reservation.id}']"
...
Then(/^I see the error message 'XY is already assigned to this contract'$/) do
  find '#flash',
       text: _('%s is already assigned to this contract') % @reservation.item.inventory_code
end
...
Then(/^no new line for this model is added$/) do
  ...
  expect(all('.line', text: @reservation.model.name).size).to be == @number_of_lines_with_item_model
end
...
Then(/^a hand over dialog appears$/) { expect(has_selector?('.modal [data-hand-over]')).to be true }

Then(/^a dialog appears$/) { expect(has_selector?('.modal')).to be true }
...
Then(/^I can enter some text in the contract note field$/) { find("textarea[name='note']") }
...
Given(/^a line with an assigned item which doesn't have a location is marked$/) do
  @reservation =
    @hand_over.reservations.where(type: 'ItemLine').find do |l|
      l.item and
        (l.item.location.nil? or (l.item.location.room.blank? and l.item.location.shelf.blank?))
    end
  @line_css = ".line[data-id='#{@reservation.id}']"
...
Given(/^there is a model with a problematic item$/) do
  @item =
    @current_inventory_pool.items.borrowable.in_stock.find do |item|
      item.is_broken? or item.is_incomplete?
    end
...
Given(/^there is a model with a retired and a borrowable item$/) do
  @model =
    @current_inventory_pool.models.find do |m|
      m.items.borrowable.where(retired: nil).exists? and m.items.retired.exists?
    end
  expect(@model).not_to be_nil
...
Given(
  /^there exists an item owned by the current inventory pool but in responsibility of pool "(.*?)"$/
) do |arg1|
  @item =
    FactoryGirl.create(
      :item,
      owner: @current_inventory_pool,
      inventory_pool: FactoryGirl.create(:inventory_pool, name: arg1)
    )
end
...
Then(/^I unfocus the option line$/) { find('body').click }
