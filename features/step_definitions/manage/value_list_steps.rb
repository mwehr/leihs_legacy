...
Given /^I open a value list$/ do
  ...
  new_window = window_opened_by { find('.modal a', text: _('Value List')).click }
    ...
    table.hashes.each do |area|
      ...
      when 'Consecutive number'
        @contract.reservations.each do |line|
          find('tr', text: line.item.inventory_code).find('.consecutive_number')
        end
      when 'Inventory code'
        reservations =
      ...
      when 'Model name'
        if @list_element[:class] == 'picking_list'
          ...
          @contract.reservations.each do |line|
            find('tr', text: line.item.inventory_code).find('.model_name', text: line.model.name)
          end
        else
          raise
        end
      ...
      when 'Quantity'
        if @list_element[:class] == 'picking_list'
          ...
          picking_lines.group_by(&:model).each_pair do |model, reservations|
            find('tr', match: :prefer_exact, text: model).find(
              '.quantity', text: reservations.sum(&:quantity)
            )
          end
        elsif @list_element[:class] == 'value_list'
          @contract.reservations.each do |line|
      ...
      when 'Price'
        @contract.reservations.each do |line|
          expect(
            find('tbody tr', text: line.item.inventory_code).find('.item_price').text.gsub(/\D/, '')
          ).to eq ('%.2f' % line.price).gsub(/\D/, '')
        end
      when 'Room / Shelf'
        find('table thead tr td.location', text: '%s / %s' % [_('Room'), _('Shelf')])
        reservations = @selected_lines_by_date ? @selected_lines_by_date : @contract.reservations
        reservations.each do |line|
          find('tbody tr', text: line.item ? line.item.inventory_code : line.model.name).find(
            '.location',
            text:
      ...
      when 'available quantity x Room / Shelf'
        find(
          'table thead tr td.location',
          text: '%s x %s / %s' % [_('available quantity'), _('Room'), _('Shelf')]
        )
        reservations = @selected_lines_by_date ? @selected_lines_by_date : @contract.reservations
        reservations.each do |line|
            ...
            if line.model.is_a?(Option)
              ...
              locations =
                line.model.items.in_stock.where(inventory_pool_id: @current_inventory_pool).select(
                  ...
                  .joins(:room)
                  .group('rooms.id', 'rooms.name', 'items.shelf')
                  .order('count DESC')
              locations.each do |location|
                if line.item_id
                  ...
                  find(
                    'tr', text: '%dx %s / %s' % [location.count, location.room_name, location.shelf]
                  )
                end
              end
            end
...
Then /^that shows the totals of the columns:$/ do |table|
  table.hashes.each do |area|
    ...
    when 'Quantity'
      expect(
        @total.find('.quantity', match: :first).has_content? @contract.total_quantity
      ).to be true
    when 'Value'
      expect(@total.find('.value', match: :first).text.gsub(/\D/, '')).to eq ('%.2f' %
           @contract.reservations.map(&:price).sum)
           .gsub(/\D/, '')
    end
  end
end
...
When(/^the models in the value list are sorted alphabetically$/) do
  names = all('.value_list tbody .model_name').map(&:text)
  expect(names.empty?).to be false
  expect(names.sort == names).to be true
end

Given(
  /^there is an order with at least two models and at least two items per model were ordered$/
) do
  @hand_over =
    @current_inventory_pool.visits.hand_over.detect do |ho|
...
When(/^each model has exactly one assigned item$/) do
  ...
  @models.uniq.each do |m|
    ...
    unless l.is_a? OptionLine
      l.update_attribute(
        :item, l.model.borrowable_items.where(inventory_pool_id: @current_inventory_pool).first
      )
    end
  end
end
...
When(/^I open the value list$/) do
  ...
  document_window = window_opened_by { click_button _('Print Selection') }
...
Then(
  /
    ^the price shown for the unassigned reservations is equal to the highest price of any of the items of that model within this inventory pool$
  /
) do
  @models.each do |m|
    reservations =
      @reservations.select do |l|
        l.is_a? ItemLine and l.model == m and not l.item.try(:inventory_code)
      end
    quantity = reservations.size
    line = all('tr', text: m.name).find { |line| line.find('.inventory_code').text == '' }
    if line
      price =
        @reservations.reload.find { |l| not l.item and l.model == m }.price_or_max_price * quantity
      formatted_price =
        ActionController::Base.helpers.number_to_currency(
          price, format: '%n %u', unit: Setting.first.local_currency_string
        )
...
Then(/^the price shown for the assigned reservations is that of the assigned item$/) do
  ...
  reservations.each do |line|
    formatted_price =
      ActionController::Base.helpers.number_to_currency(
        line.price_or_max_price, format: '%n %u', unit: Setting.first.local_currency_string
      )
...
Then(/^the unassigned reservations are summarized$/) do
  @models.each do |m|
    expect(
      all('tr', text: m.name).select { |line| line.find('.inventory_code').text == '' }.size
    ).to be <= 1 # for models with quantity 1 and an assigned item size == 0, that's why <= 1
  end
end
...
Then(/^any options are priced according to their price set in the inventory pool$/) do
  ...
  reservations.each do |l|
    ...
    formatted_price =
      ActionController::Base.helpers.number_to_currency(
        @current_inventory_pool.options.find(l.item.id).price * l.quantity,
        format: '%n %u', unit: Setting.first.local_currency_string
      )
    line.find('.item_price', text: formatted_price)
  end
end
...
Given(/^there is an order with at least two models and a quantity of at least three per model$/) do
  ...
  @order =
    FactoryGirl.create(
      :order, state: :submitted, user: customer, inventory_pool: @current_inventory_pool
    )
