...
When(/^I select a category$/) do
  ...
  find(
    "#categories #category-current a[data-type='category-current'][data-id='#{c_id}']",
    text: @category.name
  )
end

Then(/^I see that category's children$/) do
  @category.children.each { |child| find('#categories', match: :prefer_exact, text: child.name) }
end
...
Then(/^the inventory I see is filtered by this category$/) do
  within('#inventory') do
    all(".line[data-type='model']", minimum: 1).each do |model_line|
      ...
      expect(
        (model.categories.include?(@child_category) or
          @child_category.descendants.any? { |c| model.categories.include? c })
      ).to be true
    end
  end
end
...
Then(/^I can navigate back to the list of top-level categories in one single step$/) do
  ...
  Category.roots.each do |child|
    find(
      "#categories #category-list [data-type='category-filter']",
      match: :prefer_exact, text: child.name
    )
  end
end

When(/^I collapse the category filter$/) { find('#categories-toggle').click }
...
Then(/^all categories whose names match the search term are shown$/) do
  within '#categories #category-list' do
    ...
    expect(all("[data-type='category-filter']").size).to eq all(
         "[data-type='category-filter']", text: @search_term
       )
         .size
  end
end
...
Then(
  /
    ^I see a search indicator with the current search term as well the currently selected category and its children$
  /
) do
  # find('#category-root .fa.fa-search')
  # find('#category-root', text: @search_term)
  find('#category-current', text: @child_category.name)
  @child_category.children.each { |child| find('#category-list', text: child.name) }
end
...
Then(/^I can navigate to the parent category$/) { find('#category-current a').click }
...
Then(/^I see the models not assigned to any category$/) do
  ...
  within '#inventory' do
    while (all('.loading-bg').length > 0 || all('.line').length < expected_models.count)
