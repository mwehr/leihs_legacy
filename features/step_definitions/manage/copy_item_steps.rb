...
When(/^I make a note of the original inventory code$/) do
  @inventory_code_original =
    find('.row.emboss', match: :prefer_exact, text: _('Inventory Code')).find(
      'input', match: :first
    )
      .value
end
...
Then(/^I can cancel$/) { find('.button', match: :first, text: _('Cancel')) }
...
Then(/^all fields except the following were copied:$/) do |table|
  ...
  @table_hashes.each do |hash_row|
    ...
    within('.row.emboss', match: :prefer_exact, text: field_name) do
      case field_type
      when 'autocomplete'
        expect(find('input,textarea', match: :first).value).to eq (if field_value != 'None'
             field_value
           else
             ''
           end)
...
Then(/^all fields except inventory code, serial number and name are copied$/) do
  ...
  expect(
    find('.row.emboss', match: :prefer_exact, text: _('Model')).find(
      'input,textarea', match: :first
    )
      .value
  ).to eql @item.model.name
  expect(
    find('.row.emboss', match: :prefer_exact, text: _('Serial Number')).find(
      'input,textarea', match: :first
    )
      .value
      .empty?
  ...
  expect(
    find('.row.emboss', match: :prefer_exact, text: _('Name')).find('input,textarea', match: :first)
      .value
      .empty?
  ).to be true
end
...
When(/^I copy an item$/) do
  @item =
    Item.where(inventory_pool_id: @current_inventory_pool).detect do |i|
      not i.retired? and not i.serial_number.nil? and not i.name.nil?
    end
  step 'I search for "%s"' % @item.model.name
...
When(/^I am editing a(?:n) (item|software license)$/) do |arg1|
  ...
  within(
    ".group-of-lines .line[data-type='#{s2}'][data-id='#{@item.id}']", text: @item.inventory_code
...
When(/^I edit an item belonging to a different inventory pool$/) do
  @item =
    Item.find do |i|
      i.inventory_pool_id == @current_inventory_pool.id and @current_inventory_pool.id != i.owner_id
    end
