...
Then /
       ^I can find the user administration features in the "(Manage|Admin)" area under "Users"$
     / do |arg1|
  step 'I navigate to the inventory pool manage section' if arg1 == 'Manage'
  step "I open the tab '%s'" % _('Users')
end

Given /
        ^a (.*?)user (with|without) assigned role appears in the user list$
      / do |suspended, with_or_without|
  user = User.where(login: 'normin').first
  case suspended
  when 'suspended '
    user.access_rights.where(inventory_pool: @current_inventory_pool).active.first
  ...
  step 'I can find the user administration features in the "Manage" area under "Users"'
  within '#user-list' do
...
Then /^I see the following information about the user, in order:$/ do |table|
  values = table.hashes.map { |x| _(x[:en]) }
...
Then /
       ^I can change this user's information, as long as they use local database authentication and not another adapter$
     / do
...
Then /
       ^I cannot change this user's information if they use something other than local database authentication$
     / do
   if @customer.authentication_system.class_name != 'DatabaseAuthentication'
end
...
Then /^my changes are saved if I save the user$/ do
  ...
  if @customer.authentication_system.class_name == 'DatabaseAuthentication'
    @customer.reload
    @attrs.each { |attr| expect(@customer.send(attr)).to match /^#{attr}/ }
  end
end
...
Then(/^I can create new items$/) do
  c = Item.count
  attributes = { model_id: @inventory_pool.models.first.id, room_id: FactoryGirl.create(:room).id }
  expect(
    page.driver.browser.process(
      :post, manage_create_item_path(@inventory_pool, format: :json), { item: attributes }
    )
      .successful?
  ).to be true
...
Then(/^these items cannot be inventory relevant$/) do
  expect(
    page.driver.browser.process(
      :put,
      manage_update_item_path(@inventory_pool, @item.id, format: :json), {
...
Then(/^I can create options$/) do
  ...
  expect(
    page.driver.browser.process(
      :post, manage_options_path(@inventory_pool, format: :json), option: attributes
    )
      .redirection?
  ).to be true
...
Then /^I can create a new user (.*?) inventory_pool$/ do |arg1|
  ...
  attributes = {}
  [:login, :firstname, :lastname, :phone, :email, :badge_id, :address, :city, :country, :zip]
    .each { |a| attributes[a] = factory_attributes[a] }
  response =
    case arg1
    when 'for the'
      page.driver.browser.process(
        :post,
        manage_inventory_pool_users_path(@inventory_pool),
        user: attributes,
        access_right: { role: :customer },
        db_auth: {
          login: attributes[:login], password: 'password', password_confirmation: 'password'
        }
      )
    when 'without'
...
Then(/^I can create and suspend users$/) do
  ...
  expect(
    page.driver.browser.process(
      :put,
      manage_update_inventory_pool_user_path(@inventory_pool, @user, format: :json),
      access_right: { suspended_until: Date.today + 1.year, suspended_reason: 'suspended reason' }
    )
      .successful?
  ).to be true
...
Then(
  /
    ^I can assign and remove roles to and from users as specified in the following table, but only in the inventory pool for which I am manager$
  /
) do |table|
  table.hashes.map do |x|
    ...
    role =
      ...
      when _('No access')
        ...
        data = {
          user: { id: unknown_user.id },
          access_right: { role: :customer },
          db_auth: {
            login: Faker::Lorem.words(3).join,
            password: 'password',
        ...
        page.driver.browser.process(
          :put,
          manage_update_inventory_pool_user_path(@inventory_pool, unknown_user, format: :json),
          data
        )
        :no_access
      end
    ...
    data = {
      ...
      db_auth: {
        login: Faker::Lorem.words(3).join, password: 'password', password_confirmation: 'password'
      }
    }
...
Then(
  /^I can retire items if my inventory pool is their owner and they are not inventory relevant$/
) do
  item = @inventory_pool.own_items.where(is_inventory_relevant: false).first
  expect(item.retired?).to be false
  attributes = { retired: true, retired_reason: 'Item is gone' }

  expect(
    page.driver.browser.process(
      :put, manage_update_item_path(@inventory_pool, item, format: :json), item: attributes
    )
      .successful?
  ).to be true
...
Then(/^I can create new models$/) do
  ...
  expect(
    page.driver.browser.process(
      :post, "/manage/#{@inventory_pool.id}/models.json", model: attributes
    )
      .successful?
  ).to be true
...
Then(/^I can make another inventory pool the owner of the items$/) do
  attributes = { owner_id: (InventoryPool.pluck(:id) - [@inventory_pool.id]).first }
...
When(/^I don't choose a responsible department when creating or editing items$/) do
  ...
  attributes = {
    inventory_pool_id:
      (InventoryPool.pluck(:id) - [@inventory_pool.id, @item.inventory_pool_id]).first
  }
  ...
  attributes = { inventory_pool_id: nil }
  expect(@item.inventory_pool_id).not_to eq attributes[:inventory_pool_id]
...
Then(/^I can retire these items if my inventory pool is their owner$/) do
  item = @inventory_pool.own_items.find &:in_stock?
  attributes = { retired: true, retired_reason: 'retired reason' }
...
Then(/^I can remove groups$/) do
  ...
  @groups_removed.each do |group|
    find('.row.emboss', match: :prefer_exact, text: _('Groups')).find(
      '.field-inline-entry', text: group.name
    )
      .find('.clickable', text: _('Remove'))
      .click
  end
end
...
When(
  /^I am looking at the user list( outside an inventory pool| in any inventory pool)?$/
) do |arg1|
  visit case arg1
...
When(/^I add a user$/) { find_link(_('New User')).click }
...
When(/^I enter the following information$/) do |table|
  ...
  table.raw.flatten.each do |field_name|
    find(selector, match: :prefer_exact, text: field_name).find(
      'input,textarea'
...
Then(/^the user and all their information is saved$/) do
  ...
  expect(user.entitlement_groups.map(&:id)).to match_array @current_inventory_pool
                .entitlement_groups
                .map(&:id)
end
...
When(/^all required fields are filled in$/) do
  ...
  find('.row.emboss', match: :prefer_exact, text: _('E-Mail')).find(
    'input,textarea'
  ).set 'test@test.ch'
end
...
When(/^I did not enter reason for suspension$/) do
  find('.row.emboss', match: :prefer_exact, text: _('Suspended reason')).find(
    'input,textarea'
  ).set ''
end

When(/^I navigate from here to the user creation page$/) { click_link _('New User') }

Then(/^I am redirected to the user list outside an inventory pool$/) do
  expect(current_path).to eq admin.users_path
end

Then(/^the new user has been created$/) { @user = User.find_by_email 'test@test.ch' }
...
When(/^I assign the admin role to this user$/) { select _('Yes'), from: 'user_admin' }

Then(/^this user has the admin role$/) { expect(@user.reload.is_admin).to be true }
...
When(/^I remove the admin role from this user$/) { select _('No'), from: 'user_admin' }

Then(/^this user no longer has the admin role$/) { expect(@user.reload.is_admin).to be false }
...
Then(/^I can't access that page$/) { expect(current_path).not_to eq @path }
...
Then(/^I can only choose the following roles$/) do |table|
  expect(
    find('.row.emboss', match: :prefer_exact, text: _('Access as')).all('option').length
  ).to eq table.raw.length
...
Given(/^I edit a user who has access as customer$/) do
  access_right =
    AccessRight.find { |ar| ar.role == :customer and ar.inventory_pool == @current_inventory_pool }
...
Given(/^I edit a user who has access as lending manager$/) do
  access_right =
    AccessRight.find do |ar|
      ar.role == :lending_manager and ar.inventory_pool == @current_inventory_pool and
...
Given(
  /^I am editing a user who has access to (and no items from )?(the current|an) inventory pool$/
) do |arg1, arg2|
  case arg2
  when 'the current'
    ...
    @user =
      if arg1
        access_rights.detect do |ar|
          @current_inventory_pool.reservations.where(user_id: ar.user).empty?
        end
      else
        access_rights.first
      end
  ...
  when 'an'
    access_right =
      AccessRight.active.where(
        role: :customer, inventory_pool_id: @current_user.inventory_pools.managed
      )
        .detect { |ar| ar.inventory_pool.reservations.where(user_id: ar.user).empty? }
    @user = access_right.user
    @current_inventory_pool = access_right.inventory_pool
  end
  visit manage_edit_inventory_pool_user_path(@current_inventory_pool, @user)
...
When(/^I remove their access$/) do
  find('.row.emboss', match: :prefer_exact, text: _('Access as')).find('select').select _(
    'No access'
  )
end
...
Then(/^users are sorted alphabetically by first name$/) do
  within('#user-list') do
    find('.line', match: :first)
    t =
      if current_path == admin.users_path
...
When(/^I enter the login data$/) do
  ...
  find(selector, match: :prefer_exact, text: _('Password Confirmation')).find(
    'input'
  ).set 'password'
end
...
When(/^I change the email address$/) do
  selector = @current_inventory_pool ? '.row.emboss' : '.form-group'
  find(selector, match: :prefer_exact, text: _('E-Mail')).find(
    'input,textarea'
  ).set 'changed@test.ch'
end
...
Then(/^the user still has access to the current inventory pool$/) do
  expect(
    @user.access_rights.active.detect { |ar| ar.inventory_pool == @current_inventory_pool }
  ).to eq nil
end
...
Given(/^I edit a user who used to have access to the current inventory pool$/) do
  @current_inventory_pool =
    @current_user.inventory_pools.managed.where(
      id: AccessRight.select(:inventory_pool_id).where.not(deleted_at: nil)
    )
      .first
...
Then(/^inventory pools they have access to are listed with the respective role$/) do
  @user.access_rights.active.each { |access_right| find('.well ul > li', text: access_right.to_s) }
end

#Given(/^there exists a contract with status "(.*?)" for a user with otherwise no other contracts$/) do |arg1|
Given(
  /^there exists a contract with status "(.*?)" for a user without any other contracts$/
) do |arg1|
  state =
    case arg1
    when 'submitted'
  ...
  if state == :submitted
    ...
    FactoryGirl.create(
      :reservation,
      user: user, inventory_pool: @current_inventory_pool, order: @contract, status: :submitted
    )
    ...
    FactoryGirl.create(
      :reservation,
      user: user, inventory_pool: @current_inventory_pool, order: @contract, status: :approved
    )
  elsif state == :signed
...
Then(/^I see the error message "(.*?)"$/) { |arg1| find('#flash .error', text: _(arg1)) }
