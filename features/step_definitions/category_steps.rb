...
When /^I check the category "([^"]*)"$/ do |category|
  within 'ul.simple_tree' do
    all('li').each do |item|
      if item.text == category and
        item.find('input', match: :first).native.attribute('checked').nil?
...
Then /
       ^the model "([^"]*)" should( not)? be in category "([^"]*)"$
     / do |model_name, boolean, category_name|
...
Given(/^there is a main category$/) { @main_category = Category.roots.first }

Then(/^this category can have children$/) do
  child_category =
    Category.where.not(id: @main_category).detect { |c| not @main_category.children.include? c }
  @main_category.children << child_category
  expect(@main_category.reload.children.include? child_category).to be true
end

Then(/^this category has no parents$/) { expect(@main_category.parents).to be_empty }
...
After('@logoutafter') { step 'I follow the sloppy link "Logout"' }
