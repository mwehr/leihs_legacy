Then(
  /^the default (.*) exists in the database for a given (.*) and all languages$/
) do |template_name, type|
  Language.all.each do |language|
    mt =
      MailTemplate.find_by(
        is_template_template: true, type: type, name: template_name, language_id: language.id
      )
    expect(mt).to be
  end
  name_count = MailTemplate.select('DISTINCT(name)').count
  language_count = Language.count
  expect(MailTemplate.where(is_template_template: true).count).to be ==
    (name_count * language_count)
end

When(
  /
    ^I specify a mail template for the (.*) action (for the whole system|in the current inventory pool) for each active language$
  /
) do |template_name, scope|
  case scope
  ...
  when 'in the current inventory pool'
    ...
    selector2 = '.button'
    find(selector1, text: /^#{template_name}$/).find(:xpath, './..').find(
      selector2, text: _('Edit')
    )
      .click
  end
...
Then(
  /
    ^the template (.*) is saved for the (whole system|current inventory pool) for each active language$
  /
) do |template_name, scope|
  inventory_pool_id =
    case scope
...
Given(
  /^I have a contract with deadline (yesterday|tomorrow)( for the inventory pool "(.*?)")?$/
) do |day, arg1, inventory_pool_name|
  @visit =
    if arg1
...
Then(
  /^I receive an email formatted according to the (reminder|deadline_soon_reminder) mail template$/
) do |template_name|
  language = Language.find_by(locale_name: @current_user.language.locale_name)

  sent_mails =
    ActionMailer::Base.deliveries.select do |m|
      m.to.include?(@current_user.email) and m.from.include?(@visit.inventory_pool.email)
    end
  sent_mails =
    sent_mails.select do |m|
      m.subject ==
        case template_name
        when 'reminder'
          _('[leihs] Reminder')
  ...
  template =
    MailTemplate.find_by!(
      inventory_pool_id: @visit.inventory_pool_id,
      name: template_name,
      language: language,
      format: 'text'
    )
  variables =
    MailTemplate.liquid_variables_for_user(
      @current_user, @visit.inventory_pool, @visit.reservations
    )
...
When(
  /
    ^one of my submitted orders to an inventory pool without custom approved mail templates get approved$
  /
) do
  expect(ActionMailer::Base.deliveries.count).to eq 0
  @contract = @current_user.orders.submitted.detect(&:approvable?)
  @contract.approve(Faker::Lorem.sentence)
  expect(ActionMailer::Base.deliveries.count).to be > 0
end

Then(
  /^I receive an approved mail based on the system\-wide template for the language "(.*?)"$/
) do |locale_name|
  language = Language.find_by(locale_name: locale_name)

  sent_mails =
    ActionMailer::Base.deliveries.select do |m|
      m.to.include?(@current_user.email) and m.from.include?(@contract.inventory_pool.email)
    end
  ...
  template =
    MailTemplate.find_by(
      inventory_pool_id: nil, name: 'approved', language: language, format: 'text'
    )
      .body
...
When(
  /^I edit the (reminder) with the "(.*?)" template in "(.*?)"$/
) do |template_name, body, locale_name|
...
Then(/^I land on the mail templates edit page$/) do
  ...
  Language.active_languages.each do |language|
    find(
      "input[name='mail_templates[][language]'][type='hidden'][value='#{language.locale_name}']",
      visible: false
    )
  end
end
...
Then(
  /^the failing (reminder) mail template in "(.*?)" is highlighted in red$/
) do |template_name, locale_name|
  selector = @current_inventory_pool ? '.row.margin-vertical-s' : '.form-group'
  expect(
    find(selector, text: locale_name).native.css_value('background-color')
  ).to eq 'rgba(255, 176, 176, 1)'
end

Then(
  /^the failing (reminder) mail template in "(.*?)" is not persisted with the "(.*?)" template$/
) do |template_name, locale_name, body|
  language = Language.find_by(locale_name: locale_name)
  template =
    MailTemplate.find_or_initialize_by(
      inventory_pool_id: @current_inventory_pool.try(:id),
      name: template_name.gsub(' ', '_'),
      language: language,
      format: 'text'
    )
...
Then(/^I am redirected to the login page$/) { find('#login-form') }
