...
Given 'holidays are from $startdate - $finished because of $reason' do |startdate, finish, reason|
  ...
  ip.holidays <<
    Holiday.new(
      start_date: LeihsFactory.parsedate(startdate),
...
Given '$date is free because of $reason' do |date, reason|
  ...
  ip.holidays <<
    Holiday.new(
      start_date: LeihsFactory.parsedate(date), end_date: LeihsFactory.parsedate(date), name: reason
    )
  ip.save
end
