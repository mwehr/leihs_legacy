Given(/^a settings object$/) do
  ...
  @setting ||=
    Setting.create(
      {
        local_currency_string: 'GBP', email_signature: 'kthxbye', default_email: 'from@example.com'
      }
    )
end

Given(/^the settings are existing$/) { FactoryGirl.create :setting unless Setting.first }
