...
Then(/^I am informed that the remaining models are all available$/) do
  expect(
    has_content?(_('Your order has been modified. All reservations are now available.'))
  ).to be true
end

#########################################################################

Then(/^I see my order$/) { find('#current-order-lines') }
...
Then(/^the no longer available items are highlighted$/) do
  @current_user.reservations.unsubmitted.each do |line|
    unless line.available?
      find("[data-ids*='#{line.id}']", match: :first).find(:xpath, './../../..').find(
...
Then(/^I can edit entries$/) do
  all('.row.line').each { |x| x.find('button', text: _('Change entry')) }
end

Then(/^I can return to the main order overview$/) { find('a', text: _('Continue this order')) }
...
Given(/^I delete one entry$/) do
  ...
  expect(
    @line_ids.all? { |id| @current_user.reservations.unsubmitted.map(&:id).include?(id) }
  ).to be true
  ...
  @new_quantity =
    ...
    when 'increase'
      ...
      line.model.total_borrowable_items_for_user_and_pool(
        line.user, line.inventory_pool, ensure_non_negative_general: true
      )
    when 'decrease'
      1
    else
...
When(/^I correct all errors$/) do
  @line_ids.each { |line_id| resolve_conflict_for_reservation line_id }
end
...
def resolve_conflict_for_reservation(line_id)
  within ".line[data-ids='[undefined#{line_id}undefined]']" do
