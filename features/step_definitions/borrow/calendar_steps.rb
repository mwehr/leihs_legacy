When(/^I open the calendar of a model$/) do
  model =
    @current_user.models.borrowable.where('models.id' => @inventory_pool.models.select('models.id'))
      .first
...
Given(/^the current inventory pool has reached maximum amount of visits$/) do
  if @current_inventory_pool.workday.reached_max_visits.empty?
    ...
    @current_inventory_pool.workday.update_attributes(
      max_visits: (0..6).inject({}) { |h, n| h[n] = 1; h }
    )
  end
...
When(
  /
    ^I open the calendar of a model related to an inventory pool for which has reached maximum amount of visits$
  /
) do
  @inventory_pool =
    @current_user.inventory_pools.detect { |ip| not ip.workday.reached_max_visits.empty? }
  @inventory_pool ||=
...
When(
  /
    ^I open the calendar of a model related to an inventory pool for which the number of days between order submission and hand over is defined as (\d+)$
  /
) do |arg1|
  @inventory_pool =
    @current_user.inventory_pools.detect { |ip| ip.workday.reservation_advance_days == arg1.to_i }
  @inventory_pool ||=
    begin
...
Then(/^(the|no) availability number is shown (.*)$/) do |arg1, arg2|
  ...
  within '.modal' do
    Array(dates).each do |date|
      while has_no_selector?(".fc-widget-content[data-date='#{date}']")
...
Then /
       ^the (start|end) date in the booking calendar becomes red and I see a (closed|not possible|too early) day warning?$
     / do |arg1, arg2|
