...
Given(/^I am listing the main categories$/) { visit borrow_root_path }

Then(/^I stay on listing the main categories$/) { expect(page.current_path).to eq borrow_root_path }
...
When(/^I hover over a main category with children$/) do
  ...
  within('[data-category_id]', text: @main_category.name) { find('.dropdown-holder').hover }
end
...
Then(/^I see only this main category's children that are useful and available to me$/) do
  ...
  visible_2nd_level_categories =
    (@current_user.categories.with_borrowable_items & @main_category.children)
  ...
  within find('[data-category_id] .padding-inset-s', match: :first, text: @main_category.name)
           .first(:xpath, '../..')
           .find('.dropdown-holder', match: :first) do
...
Then(/^that main category has no child category dropdown$/) do
  expect(
    find(
      '.row.emboss.focus-hover', match: :first, text: @main_category.name
    ).has_no_selector? '.dropdown-holder'
  ).to be true
end
...
Then(
  ...
  @main_categories.each do |mc|
    ...
    expect(response.location).to match /#{image.id}/ if image = mc.image
  end
end
