  ...
  module Spec
    module OrderMailSteps
      ...
      step 'I have access to pool :letter' do |letter|
        ...
        user: @customer, inventory_pool: ivar_get("@pool_#{letter}"), role: :customer
      end

      step 'pool :pool_letter has a borrowable item for model :model_letter' do |pool_letter, model_letter|
        FactoryGirl.create(
          :item,
      ...
      step 'one mail with received template was sent to pool :letter' do |letter|
        m =
          ActionMailer::Base.deliveries.find do |mail|
            mail.to.first == ivar_get("@pool_#{letter}").email and mail.subject.match(/received/)
          end
        expect(m).to be
      end
      ...
      step 'one mail with submitted template for pool :letter ' \
        ...
        m =
          ActionMailer::Base.deliveries.find do |mail|
            mail.to.first == @customer.email and mail.subject.match(/submitted/) and
...
RSpec.configure { |config| config.include Borrow::Spec::OrderMailSteps, borrow_order_mails: true }
